...
def supprimer_client(ftp, agency_id, site):
    if site == 'figaro':
        ...
        st.error(f"Site '{site}' non valide pour la suppression.")
        return
    ...
    if not found:
        st.warning(f"L'ID d'agence {agency_id_str} n'a été trouvé dans aucun fichier du site '{site}'.")
...
def modifier_client(ftp, agency_id, site, new_contact_mode):
    if site == 'figaro':
        ...
        st.error(f"Site '{site}' non valide pour la modification.")
        return
...
def check_coherence(results, site_name):
    ...
    if has_global and has_split:
        ...
        msg = f"⚠️ Configuration {site_name} INCOMPLÈTE : Présent dans Global mais manquant dans les fichiers scindés."
        st.error(msg)
    elif not has_global and has_split:
        msg = f"⚠️ Configuration {site_name} INCOMPLÈTE : Présent dans un fichier scindé mais manquant dans Global."
        st.error(msg)
